...
| `imu/mag` | `sensor_msgs/MagneticField` | Magnetometer (MLX90393) x/y/z raw values |
| `light` | `std_msgs/Float32` | Ambient light level (VEML7700) in lux |

### Parameters (Dynamic Tuning)

The node supports real-time calibration via standard ROS 2 parameters. Changes are persisted to the STM32's internal Flash after a 2-second settle period.

| Parameter | Type | Description |
|-----------|------|-------------|
| `servo_offset_1`..`5` | `double` | Zero-point offset in degrees. (Physical 90° = 90 + offset) |

**Example Usage:**
```bash
# Adjust Servo 1 offset
ros2 param set /sensor_servo_board servo_offset_1 -5.5

# Save happens automatically on the board
```
...
|-------|-------|------|--------|
| 0–4 | ToF distance 0–4 | meters | VL53L0X/L1X (converted from mm) |
